...
impl KNN {
    ...
    pub fn predict(&self, point: &Point) -> String {
        classify(point, &self.features, self.k).0
    }
}

pub fn classify(point: &Point, features: &[SampleWithFeatures], k: usize) -> (String, Vec<SampleWithFeatures>) {
    let sample_points = features
        .iter()
        .map(|x| Point {
            x: x.point[0],
            y: x.point[1],
    ...
    let indices = point.get_nearest_k(&sample_points, k);
